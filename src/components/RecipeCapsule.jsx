    ...
    <div className="flex justify-end w-full sm:h-60 lg:h-100 bg-zinc-800 rounded-full">
      ...
      >
        <img
          src={image}
          alt={name}
          className="sm:h-60 md:h-65 lg:h-100  object-contain rounded-full"
        />
      </div>

      <div className="flex flex-col lg:text-lg justify-center flex-1 p-4 gap-3 text-white ml-5">
        <p
        ...
        <p
          className={`flex items-center gap-2 text-sm lg:text-lg text-zinc-300 ${
            isEven ? "justify-end mr-12" : "justify-start"
          }`}
        >
          <IoIosTimer className="lg:text-lg" />
          {cookTimeMinutes} min
        </p>

        <p
          className={`flex items-center gap-2 text-sm text-zinc-300 lg:text-lg ${
            isEven ? "justify-end mr-12" : "justify-start"
          }`}
        >
          <PiCarrotLight className="lg:text-lg" />
        ...
        >
          ...
          <button
            ...
            title={`${!hasFavorite ? "add to favorites" : "remove from favorites"}`}
            onClick={() => {
              toggleFavorite(item);
            }}
          >
